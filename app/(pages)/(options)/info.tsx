...
const InfoPage = () => {
  ...
  useEffect(() => {
    const fetchHost = async () => {
      try {
        const h = await SecureStore.getItemAsync("host") || null
        setHost(h);
    ...
    const fetchProfile = async (host: string | null) => {
      try {
        axios.get(`${host}/api/personal`).then((result) => {
          setName(result.data.data.name);
          setAge(result.data.data.age.toString());
          setGender(result.data.data.gender);
          setType(result.data.data.category);
    ...
    fetchHost().then((fetchedHost) => fetchProfile(fetchedHost));
  }, []);
